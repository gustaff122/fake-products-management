...
export const hasDraftMarketingNameGuard: CanActivateFn = (_route: ActivatedRouteSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree => {
  const router = inject(Router);
  const specialOffersService = inject(SpecialOffersService);

  const draft = specialOffersService.getSpecialOfferDraft();

  if (draft && draft.definition.description.marketingName) {
    return true;
  } else {
